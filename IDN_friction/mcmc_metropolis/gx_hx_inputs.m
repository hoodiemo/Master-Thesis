...
%M: I added the iteration term, to evalute some parameters when running the
%M: model for the last round (for efficiency, as eval() takes way too long
%M: otherwise
%
...
% check if iteration argument is provided, set iteration to 0 if not
if nargin == 1
    iteration = 0;
end
...
G =    1.022053; %mean growth rate of output in Indonesia for the period 1870 to 2020 excluding WW2 period. 

DELTA = 0.04; %Depreciation rate -> to get inv % in gdp of 14.2
...
tby = 0.01195; %trade balance to output ratio 
