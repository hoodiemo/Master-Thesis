...
D =[     0    0.2000 %sigmag
   ...
   -0.9900    0.9900 %rhomu
         ...
         0    10.0000%pssi
         0    5.0000 %eta
    0.0001    0.0105 %STDmey
    0.0001    0.0147 %STDmec
    0.0001    0.0468 %STDmeiv
    0.0001    0.0101 %STDmetby
    ];
%Elements D(14:17,2) were obtained as std(Y)'*0.25;  This implies that the upper bound of the prior distribution of the standard  deviation of measurement errors is 25 percent of the standard deviation of the observables.  Equivalently, measurement errors are allowed to explain at most 6.25 percent of the variance of the observable. 
...
% Check before doing Choleski Decomposition
eig_values = eig(var_param);
if any(eig_values <= 0)
    epsilon = 1e-10;
    var_param = var_param + epsilon * eye(size(var_param));
    disp('Adjusted var_param to make it positive definite');
end

std_param = chol(var_param,'lower');

eval(['save bayes.mat param_chain logposterior_chain exitflag_chain std_param0  mean_param var_param accept_rate LB UB k Y1 Y'])